...
ggplot(data = stayxrating2, aes(x = Overall.Rating, y = SNF.Average.Length.of.Stay.Total)) +
  geom_point(aes(color = Overall.Rating)) + 
...
stayxrating2 <- stayxrating %>%
  filter(!is.na(Overall.Rating) & !is.na(SNF.Average.Length.of.Stay.Total)) %>%
  mutate(Rating_Category = cut(Overall.Rating, breaks = 5)) # Adjust breaks as needed
